        ...
        foreach (var fi in files)
        {
            ...
            if (categoryNames.Length > 0)
            {
                ...
                if (rootCategory == null)
                {
                    ...
                    rootCategory = categoryRepo.Where(a => a.Name == categoryNames[0]).First();
                    rootCategory.Categories = categoryService.GetCategoryHierarchy(rootCategory.Id);
                    categoryRepo.Update(rootCategory);
                }
                ...
                for (var i = 1; i < categoryNames.Length; i++)
                {
                    ...
                    if (category == null)
                    {
                        ...
                        category = categoryRepo.Where(a => a.ParentId == parent.Id && a.Name == name).First();
                        category.Categories = categoryService.GetCategoryHierarchy(category.Id);
